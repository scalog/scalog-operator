...
// blank assignment to verify that ReconcileScalogService implements reconcile.Reconciler
...
func (r *ReconcileScalogService) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.List(context.Background(), &dataShardSelector, existingDataShards); err == nil {
		...
		if instance.Spec.NumShards == currSize {
			...
			if instanceErr := r.client.Status().Update(context.Background(), instance); instanceErr != nil {
			...
			finalReq := &om.FinalizeEntry{ShardIDs: []int32{sid}}
	...
	// // Set ScalogService instance as the owner and controller
